...
export class SellerController {
  ...
  async orders(@Query() query: OrderQueryDto, @Req() request: Request): Promise<Paginated<Order>> {
    const seller = request.seller;
    if (!seller) throw new Error('SELLER_CONTEXT_MISSING');
  ...
  async order(@Param('id') id: string, @Req() request: Request): Promise<Order> {
  ...
  async payments(@Query() query: never, @Req() request: Request) {
    const seller = request.seller;
    if (!seller) throw new Error('SELLER_CONTEXT_MISSING');
    void seller;
